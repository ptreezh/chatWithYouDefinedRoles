...
class DeploymentManager {
    ...
    loadConfig() {
        ...
        return {
            deployment: {
                type: 'local', // local, pm2, docker, vercel
                domain: '',
                ssl: false,
                ollama: true
            },
    ...
    async checkPrerequisites() {
        ...
        try {
            ...
            this.log('npm未安装或不可用', 'error');
            return false;
        }

        // 检查项目文件
        const requiredFiles = ['package.json', 'server.ts', 'src/app/page.tsx'];
...
DATABASE_URL=${this.config.database.url || 'file:./db/custom.db'}
...
`;
    ...
    async deployLocal() {
        ...
        if (this.config.deployment.ollama) {
            this.log('启动Ollama服务...');
            try {
                execSync('ollama serve > /dev/null 2>&1 &', { stdio: 'ignore', cwd: this.projectDir });
        ...
        // 启动应用
        try {
            execSync('npm start > server.log 2>&1 &', { stdio: 'ignore', cwd: this.projectDir });
